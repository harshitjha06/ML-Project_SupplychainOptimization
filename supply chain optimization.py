...
def get_existing_outward_distance(warehouse, vendors, shops, branchcolname):
    ...
    customer = shops.copy()
    network_graph = get_nxgraph(warehouse, vendors, shops, branchcolname, True)
...
def get_mdc(df_vendors, df_shops, start_point):
    centers_old = None
    for rand in range(101, 103):
        ...
        for index, c in centers.iterrows():  # iterating through the 2 centers
            ...
            clong = c['Lon']
            ds = []
            for ind, p in df_shops.iterrows():  # iterating through the customer dataset
                plat = p['Lat']
                plong = p['Lon']
            ...
            centers.at[index, "Lat"] = df_shops.at[idx, "Lat"]  # putting centers as the nearest customer
            centers.at[index, "Lon"] = df_shops.at[idx, "Lon"]
            centers.at[index, "Region"] = df_shops.at[idx, "Region"]
            centers.at[index, "City"] = df_shops.at[idx, "City"]
        ...
        if centers_old is None:
            ...
            for index2, c2 in centers_old.iterrows():  # iterating through the 2 centers
                clat = c2["Lat"]
                clong = c2['Lon']
                for ind, p in df_vendors.iterrows():  # iterating through the vender dataset
                    plat = p['Lat']
                    plong = p['Lon']
                    d = distance(clat, clong, plat, plong)
            ...
            for index1, c1 in centers_new.iterrows():  # iterating through the 2 centers
                clat = c1["Lat"]
                clong = c1['Lon']
                for ind, p in df_vendors.iterrows():  # iterating through the vendor dataset
                    plat = p['Lat']
                    plong = p['Lon']
                    d = distance(clat, clong, plat, plong)
...
def get_cluster(df_shops, df_dcs, start_point, rand):
    df_old = None
    for rand in range(rand, rand+1):  # random state variable
        ...
        for index, c in centers.iterrows():
            ...
            clong = c['Cluster Lon']
            ds = []
            for ind, p in df_shops.iterrows():
                plat = p['Lat']
                plong = p['Lon']
                d = distance(clat, clong, plat, plong)
                ds.append(d)
            ...
            centers.at[index, "Cluster Lat"] = df_shops.at[idx, "Lat"]
            centers.at[index, "Cluster Lon"] = df_shops.at[idx, "Lon"]
            centers.at[index, "Cluster City"] = df_shops.at[idx, "City"]  # marking k warehouses as the closest customer location
...
FILE_NAME = "C:\\Users\\harsh\\Documents\\5th semester\\ML\\project\\ADA project\\Initial_Data.xlsx"
...
for k in range(10, 31):
    ...
    kth_wss = 0
    for index, c in df_old.iterrows():
        clat = c['Cluster Lat']
        clon = c['Cluster Lat']
        plat = c['Lat']
        plon = c['Lon']
        dis = distance(clat, clon, plat, plon)
        power_dis = dis*dis
        kth_wss = kth_wss + power_dis
...
plt.scatter(wss_k, wss, color='r', zorder=1)
plt.plot(wss_k, wss, color='b', zorder=2)
