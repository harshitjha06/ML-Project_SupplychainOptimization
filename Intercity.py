...
def find_tsp(centers, df_tot, df_final):
    ...
    df_path = pd.DataFrame(columns=['source', 'slat', 'slong', 'target', 'tlat', 'tlong', 'auth_name'])
    for source, g in dict_g.items():
        path = find_best_path(g)
        for index in range(len(path) - 1):
            df = {'slat': df_tot[(df_tot.identity == path[index])].iloc[0].Lat,
                  'slong': df_tot[(df_tot.identity == path[index])].iloc[0].Long,
                  'tlat': df_tot[(df_tot.identity == path[index + 1])].iloc[0].Lat,
                  'tlong': df_tot[(df_tot.identity == path[index + 1])].iloc[0].Long,
                  'source': path[index], 'target': path[index+1], 'auth_name': source}
...
file_name_cit = "D:\\Documents\\3rd semester\\Discrete mathematics\\project\\citizen.xlsx"
file_name_auth = "D:\\Documents\\3rd semester\\Discrete mathematics\\project\\auth.xlsx"
...
df_final.to_excel("D:\\Documents\\3rd semester\\Discrete mathematics\\project\\clustured dataset.xlsx")
G = nx.Graph()
df_path = find_tsp(centers, df_tot, df_final)
df_path.to_excel("D:\\Documents\\3rd semester\\Discrete mathematics\\project\\output_df.xlsx")
